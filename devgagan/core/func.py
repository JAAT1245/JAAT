   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await message.reply_text("You are Banned. Contact -- @courses_hub2_bot")
            return 1
      except UserNotParticipant:
        caption = f"Join our channel to use the bot"
        await message.reply_photo(photo="https://i.ibb.co/wZPLSVKc/photo-2025-05-03-06-52-33-7500108789786345476.jpg",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]]))
        return 1
      except Exception:
         await message.reply_text("Something Went Wrong. Contact us @targetallcourse...")
         return 1
...
async def progress_callback(current, total, progress_message):
    ...
    f"╰──────────────────╯\n\n"
    f"**__Powered by Team CR CHOUDHARY ✅**"
        )
